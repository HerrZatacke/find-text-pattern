...
let loadedRomArrayBuffer;
let romPage = 240;
const romSize = 0x100000;
const pageSize = 0x200;
// const pageSize = 0x1000;
const maxPage = romSize / pageSize;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const showCurrentRomPage = () => {
    if (!loadedRomArrayBuffer) {
      return;
    }

    const bufferPart = loadedRomArrayBuffer.slice(romPage * pageSize, (romPage + 1) * pageSize);
    const view = new Uint8Array(bufferPart);
  ...
  input.addEventListener('keyup', (ev) => { ev.stopPropagation(); update(); })
  input.addEventListener('blur', update)
  input.addEventListener('change', update)
  romfile.addEventListener('change', async (ev) => {
    const arraybuffer = await ev.target?.files?.[0]?.arrayBuffer?.();
    // Valid ROM
    if (arraybuffer?.byteLength === romSize) {
      loadedRomArrayBuffer = arraybuffer;
      romPage = 0;
      showCurrentRomPage();
    }
  })
